...
class LaravelLogViewer
{
    ...
    {
        preg_match("/\d{4}-\d{2}-\d{2}/", basename(self::$file), $match);
		if(!isset($match[0])) return null;
		return  $match[0];
    }
                ...
                foreach (self::$log_levels as $level) {
                    if (strpos(strtolower($h[$i]), '.' . $level) || strpos(strtolower($h[$i]), $level . ':')) {
                        ...
                        $log[] = array(
                            ...
                            'full_date' => $current[1],
                            'date' => array_get(explode(' ', $current[1]), 0, false),
                            'time' => array_get(explode(' ', $current[1]), 1, false),
                            'text' => $current[4],
    ...
    public static function getFiles($basename = false)
    {
        ...
        if ($basename && is_array($files)) {
            foreach ($files as $k => $file) {
				...
				$count = preg_match("/\d{4}-\d{2}-\d{2}/", basename($file), $match);
				if(isset($match[0])) 
					$files[$k] = $match[0];
            }
        }
