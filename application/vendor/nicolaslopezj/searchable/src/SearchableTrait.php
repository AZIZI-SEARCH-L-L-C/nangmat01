...
trait SearchableTrait
{
     ...
     */
    public function scopeSearch(Builder $q, $search, $wordLength = 2, $threshold = null, $entireText = false, $entireTextOnly = false)
    {
        return $this->scopeSearchRestricted($q, $search, $wordLength, null, $threshold, $entireText, $entireTextOnly);
    }

    public function scopeSearchRestricted(Builder $q, $search, $wordLength, $restriction, $threshold = null, $entireText = false, $entireTextOnly = false)
    {
        $query = clone $q;
        $query->select($this->getTable() . '.*');
        $this->makeJoins($query);

        if ( ! $search ) return $q->where('id', -1);

        $search = mb_strtolower(trim($search));
        $words = explode(' ', $search);
		
		foreach($words as $key => $word){
			if(strlen($word) <= $wordLength) unset($words[$key]);
		}
		
		if( empty($words) ) return $q->where('id', -1);
